...
> invocar `tdd_tester`.
>
> **Nota de retro-ajuste (`D-031`):** la sección §5 «Tareas» (atómicas, codificadas `TSK-xx`, con
> responsable, estado y trazabilidad → `CA-xx`) y el mapa caso → tarea se añadieron retroactivamente
> al adoptar la trazabilidad codificada; las referencias `ACn` de los casos pasan a `CA-0n`.
...
- **YAML:** el `client.yaml` de esta versión tiene solo dos claves escalares. Se escribe de forma
  determinista. Para **leerlo/parsearlo en los tests** (CA-06) se necesita un parser YAML. Ver
...
> Cada tarea es **atómica** y respeta las reglas de partición (`D-031`): **un solo responsable**, **un
> solo entregable**, y **código y test en tareas separadas**. **Estado** inicial `no_implementada`
...
|---|---|---|---|---|---|
| TSK-01 | Andamiaje mínimo de paquete: `pyproject.toml` (paquete `foda`, layout `src/`, pytest, `python>=3.13`, dep PyYAML) + `__init__.py` de `src/foda/`, `src/foda/core/`, `tests/`. | Paquete importable/testeable | tdd_coder | no_implementada | (andamiaje; `D-027`) |
| TSK-02 | Implementar en `create_client` la creación del árbol (medallion + `models/` + `010_inputs/` + `020_outputs/`) y el retorno del `Path`. | `scaffold.py` (creación de árbol) | tdd_coder | no_implementada | CA-01, CA-02, CA-03, CA-04, CA-05, CA-07, CA-09 |
| TSK-03 | Implementar la escritura de `client.yaml` con `name` y `created_at` (UTC, ISO `YYYY-MM-DD`). | `scaffold.py` (`client.yaml`) | tdd_coder | no_implementada | CA-06 |
| TSK-04 | Implementar la validación del nombre (patrón DS-1) con `ValueError`, **antes** de tocar disco. | `scaffold.py` (validación) | tdd_coder | no_implementada | CA-08, CA-11 |
| TSK-05 | Implementar la comprobación de duplicado con `FileExistsError`, **antes** de tocar disco. | `scaffold.py` (duplicado) | tdd_coder | no_implementada | CA-10, CA-11 |
| TSK-06 | Implementar el rollback best-effort ante fallo de FS (DS-2.2). **Sin test en esta banda** (`D-028`). | `scaffold.py` (rollback) | tdd_coder | no_implementada | (DS-2.2; caso 18) |
| TSK-07 | Cablear la capa CLI `foda client new <NAME>` sobre el core. **Sin test en esta banda** (spec §No-Objetivos). | `cli.py` | tdd_coder | no_implementada | (contrato CLI) |
| TSK-08 | Escribir el test del árbol creado + retorno de `Path` (casos 1–5). | test(s) de creación | tdd_tester | no_implementada | CA-01, CA-02, CA-03, CA-04, CA-05, CA-07 |
| TSK-09 | Escribir el test de `client.yaml` (`name`, `created_at`) (casos 6–7). | test `client.yaml` | tdd_tester | no_implementada | CA-06 |
| TSK-10 | Escribir el test de nombres válidos representativos (caso 8). | test nombres válidos | tdd_tester | no_implementada | CA-09 |
| TSK-11 | Escribir el test parametrizado de nombres inválidos → `ValueError`, nada creado (casos 9–16). | test nombres inválidos | tdd_tester | no_implementada | CA-08, CA-11 |
| TSK-12 | Escribir el test de duplicado → `FileExistsError`, contenido intacto (caso 17). | test duplicado | tdd_tester | no_implementada | CA-10, CA-11 |
| TSK-13 | Refactor: consolidar los tests de nombres inválidos en `pytest.mark.parametrize` y limpiar duplicación, manteniendo verde. | Refactor (sin cambio de comportamiento) | tdd_refactor | no_implementada | (mejora de CA-08) |
...
aceptación (`CA-xx`) de la spec entre paréntesis.

1. `create_client("ABC", tmp)` crea el directorio `tmp/ABC/` y devuelve un `Path` que apunta a él. (CA-01, CA-07)
2. Bajo `tmp/ABC/` existen exactamente estas entradas de primer nivel: archivo `client.yaml` y carpetas `010_inputs/`, `020_outputs/`, `data/`, `models/`. (CA-02)
3. `tmp/ABC/010_inputs/` y `tmp/ABC/020_outputs/` existen y están vacías. (CA-05)
4. `tmp/ABC/data/` contiene `bronze/`, `silver/`, `gold/`, y las tres están vacías. (CA-03)
5. `tmp/ABC/models/` existe y está vacía (sin subcarpetas de versión). (CA-04)
6. `tmp/ABC/client.yaml` es YAML válido que parsea a un mapa cuya clave `name == "ABC"`. (CA-06)
7. `tmp/ABC/client.yaml` tiene `created_at` cuyo valor cumple el patrón `^\d{4}-\d{2}-\d{2}$`. (CA-06)
8. Para cada nombre válido representativo (`"X"`, `"9lives"`, `"Client_1-a"`), `create_client(...)` crea el árbol completo sin error. (CA-09)
9. Nombre vacío `""` lanza `ValueError` y no crea ninguna carpeta. (CA-08, CA-11)
10. Nombre solo-espacios `"   "` y con espacio interior `"ab c"` lanzan `ValueError`; nada creado. (CA-08, CA-11)
11. Nombre que empieza por guion `"-abc"` o por guion bajo `"_abc"` lanza `ValueError`; nada creado. (CA-08, CA-11)
12. Nombre con separador de ruta `"a/b"` o `"a\\b"` lanza `ValueError`; nada creado. (CA-08, CA-11)
13. Nombres de ruta especiales `"."` y `".."` lanzan `ValueError`; nada creado. (CA-08, CA-11)
14. Nombre con punto interior `"ab.c"` o carácter no permitido (`"a!b"`, `"a@b"`) lanza `ValueError`; nada creado. (CA-08, CA-11)
15. Nombre no-ASCII / acentuado (`"añez"`, `"clienté"`) lanza `ValueError`; nada creado. (CA-08, CA-11)
16. Nombre de longitud > 64 (65 caracteres válidos) lanza `ValueError`; nada creado. (CA-08, CA-11)
17. `create_client("ABC", tmp)` cuando `tmp/ABC/` ya existe (con un archivo centinela dentro) lanza `FileExistsError` y deja el contenido preexistente intacto. (CA-10, CA-11)
...
Cada caso agrupa su tarea-test y su tarea-código (el bucle corre por caso; las tareas son la capa de trazabilidad).
...
| 9–16 | TSK-11 | TSK-04 |
| 17 | TSK-12 | TSK-05 |
| 18 | *(sin test, `D-028`)* | TSK-06 |

> Nota de granularidad: los casos 9–16 se cubren con un único test parametrizado (TSK-11) sobre la
> tabla de nombres inválidos de la spec; se enumeran por separado por trazabilidad y se consolidan en
> el refactor (TSK-13) sin perder cobertura. El cableado CLI (TSK-07) no tiene caso de test en esta banda.
...
## 8. Puntos abiertos para el GATE humano (NC-6)

- **PA-1 — Parser/serializador YAML.** El core escribe un `client.yaml` de dos claves escalares (se
  puede hacer sin dependencias). Pero **leerlo en los tests** (CA-06) requiere un parser. Propuesta:
