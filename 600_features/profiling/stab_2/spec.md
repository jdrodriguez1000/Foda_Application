...
- **A-021 RESUELTA → pesos por defecto 25 % cada sub-score (`0.25` × 4), SOBREESCRIBIBLES por cliente** en `profiling_config.yaml`. La spec fija tanto los defaults como el mecanismo de override (`DS-PRF2-1`, `DS-PRF2-6`).
...
## DS-PRF2-6 — Artefacto de configuración por cliente `profiling_config.yaml` (`D-100`, A-021)

- **Ubicación:** `clients/<name>/010_inputs/040_profiling/profiling_config.yaml` (`system_design.md` §7: `010_inputs` aloja los YAML de decisión humana por flujo; coherente con `010_discovery`, `050_cleaning/data_cleaner.yaml`). Vía `Artifact(base="inputs", relative="040_profiling/profiling_config.yaml")`.
...
- **Formato (YAML):**
  ```yaml
  # clients/<name>/010_inputs/040_profiling/profiling_config.yaml
...
| requiere (opcional) | `profiling_config` | `Artifact(base="inputs", relative="040_profiling/profiling_config.yaml")` | YAML | Catálogo de centinelas + pesos. Opcional: si falta, catálogo vacío y pesos default (`DS-PRF2-6`). **No** en `requires` de `validate`. |
...
| CA-17 | Profiling lee el catálogo de `clients/<name>/010_inputs/040_profiling/profiling_config.yaml`, unificando las cajas `numeric`, `non_numeric` y `boolean` en un solo conjunto de tokens; `defects.validity.catalog_size` == nº de tokens de esa unión. | HU-05 |
...
- **Ampliación de `ClientContext`/`Artifact`** más allá de referenciar `010_inputs/040_profiling/profiling_config.yaml` (base `inputs`, ya soportada por el core).
...
6. **`DS-PRF2-6` — `profiling_config.yaml`.** ¿Se acepta su ubicación `010_inputs/040_profiling/profiling_config.yaml`, su carácter **opcional** (fuera de `requires`), y el formato con bloques `sentinels{numeric,non_numeric,boolean}` y `weights`?
