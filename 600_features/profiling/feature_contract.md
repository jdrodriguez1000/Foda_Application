...
| `stab_1` *(CONFORME, cerrada en `main`, T-039, D-088/D-089)* | Endurecimiento: cálculo real de la salud **ESTRUCTURAL** del ingreso, derivada **únicamente** de `ingestion_report.json` (sin leer `bronze/`): `global_score`, conteos (`files_declared`/`files_healthy`/`files_with_problems`), desglose `problems_by_type` y ranking `pareto`. Enriquece `profiling_report.json` con un bloque `health` y sube `schema_version` a `"0.2"`. Diferido a bandas posteriores: salud a nivel de datos/celda (leer `bronze/`: nulos, duplicados, tipos, rangos — `stab_2`), comparación contra `client_register.yaml` real, exportables csv/xlsx. |
| `stab_2` *(en curso, T-040, D-097/D-098/D-099/D-100)* | "Defectos con score": primera lectura del **contenido** de `bronze/` (nulos, filas duplicadas, valores centinela) — defectos **inequívocos** que penalizan el score (`D-098`). Rediseña `global_score` a modelo **multi-dimensión**: media ponderada de 4 sub-scores normalizados a [0,1] — `structural_score` (el `global_score` de `stab_1`, reinterpretado como una dimensión más), `completeness_score` (1 − celdas_nulas/celdas_totales), `validity_score` (1 − celdas_centinela/celdas_totales), `uniqueness_score` (1 − filas_duplicadas/filas_totales) (`D-099`). Introduce el artefacto de configuración por cliente `profiling_config.yaml` (catálogo de valores centinela por cliente; catálogo vacío si el cliente no lo define, `D-100`). Diferido a `stab_3`: perfil diagnóstico por columna (categórico + numérico), informativo, no penaliza el score (`D-097`/`D-098`). |
| `stab_3` *(backlog, T-041, D-097/D-101/D-102)* | "Perfil diagnóstico por columna": informativo, NO penaliza `global_score` (`D-098`). Perfil categórico (distribución/dominancia, columnas constantes, cardinalidad, candidatos a casi-duplicado por variantes tipográficas — Caso A detectable, Caso B equivalencia semántica fuera de alcance, `D-101`) + perfil numérico (min/max/media/mediana/cuartiles + atípicos por IQR/Tukey excluyendo nulos/centinelas antes de calcular cuartiles, `D-102`). |
...
- `800_persistence/decisions.md`: `D-080` (gate de progresión + exit code), `D-079`/`D-081` (política de ramas y PR), `D-088`/`D-089` (alcance base `stab_1`), `D-097`/`D-098`/`D-099`/`D-100` (reparto de bandas `stab_2`/`stab_3`, principio "defectos con score" vs. "perfil diagnóstico", modelo multi-dimensión de `global_score`, `profiling_config.yaml`), `D-101`/`D-102` (alcance `stab_3`: consistencia categórica, atípicos numéricos).
