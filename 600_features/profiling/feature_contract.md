...
| `stab_1` *(prevista, no iniciada)* | Endurecimiento: implementación real de la lógica de salud de datos (indicador global %, desglose por tipo de problema, pareto), lectura efectiva de `bronze/`, exportables csv/xlsx, y comparación contra `client_register.yaml` real cuando Discovery exista. |
