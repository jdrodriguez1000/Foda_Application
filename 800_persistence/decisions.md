...
| D-078 | `ingestion` — DS-ING-9: enmienda de DS-ING-2, se añade una lista top-level `inconsistencies[]` al reporte que agrega todas las inconsistencias (incl. `unexpected_file`) con `type`+`detail`; salda la deuda de los casos 11/12 | Aceptada | 2026-07-08 |
...
### D-061 — Cierre CONFORME de `onboarding` (banda `tracer_bullet`) con hallazgo no bloqueante OBS-1
- **Estado:** Aceptada
- **Fecha:** 2026-07-06
- **Contexto:** Tras cerrar el bucle TDD (22/22 casos, 116 tests) y correr `integration_tester` (5 tests de integración nuevos, 121 passed), `spec_verifier` recorrió los 22 CA de `spec.md` de `onboarding` buscando evidencia de test/comportamiento para cada uno.
- **Decisión:** Veredicto **CONFORME**: los 22 CA (CA-01…CA-21, incl. CA-05b) tienen evidencia verificable en la suite, documentada en `600_features/onboarding/tracer_bullet/verification.md` con matriz de trazabilidad completa; cero parciales, cero sin cubrir. In scope de `definition.md` cumplido, out of scope respetado (sin Discovery/Ingestion reales, sin escritura en bronze/silver/gold, sin LLM, sin ampliar el core); restricciones satisfechas (determinismo byte a byte, `FlowContractError` único de dominio, validación explícita sin JSON Schema/Pydantic). Se registra 1 hallazgo NO bloqueante: **OBS-1** — la regla "`field.required` es booleano" figura en el Comportamiento Esperado 2b de `spec.md` pero no está codificada como CA ni tiene test ni implementación explícita; no es no conformidad porque los 22 CA-xx (fuente de verdad) están todos cubiertos.
- **Consecuencias:** `onboarding/tracer_bullet` queda cerrada; es la QUINTA feature en recorrer la cadena de 8 agentes de punta a punta CONFORME. OBS-1 queda como candidato de endurecimiento para una banda `stab_1` futura (nueva tarea backlog T-029: añadir CA formal + bucle TDD vía `spec_writer`).

### D-062 — Próxima feature: `flow_orchestrator` (orquestador `foda run`/`foda status`), banda `tracer_bullet` acotada
- **Estado:** Aceptada
- **Fecha:** 2026-07-06
...
### D-067 — Cierre CONFORME de `flow_orchestrator` (banda `tracer_bullet`), sexta feature completa
- **Estado:** Aceptada
- **Fecha:** 2026-07-07
...
### D-078 — `ingestion`: DS-ING-9, lista top-level `inconsistencies[]` en el reporte (decisión del humano al aflorar la deuda de los casos 11/12 en el caso 16)
- **Estado:** Aceptada
- **Fecha:** 2026-07-08
- **Contexto:** El esquema DS-ING-2 (D-071), aprobado en el GATE de spec, aloja las `inconsistencies[]` **solo** anidadas dentro de `datasets[].files[]`. Pero un archivo **sobrante** (presente no declarado en el contrato, caso 12/CA-07) **no pertenece a ningún dataset**, por lo que no tiene una entrada `datasets[].files[]` donde alojar su inconsistencia. Al mismo tiempo, la spec declara un **vocabulario cerrado de 4 tipos** que incluye `unexpected_file` (spec §Contratos línea 141) y el comportamiento de `execute` dice "Presente-no-esperado → entra en `unexpected_files` **+ inconsistencia `unexpected_file`**" (línea 154), y CA-16 exige que **cada** inconsistencia tenga `type` del vocabulario cerrado y `detail` no vacío. Esta contradicción se documentó como deuda en los casos 11/12 (`state.json`) y el `tdd_tester` la elevó (NC-1/NC-6) al llegar al caso 16 en vez de asumir.
- **Decisión:** Se **enmienda DS-ING-2**: el reporte gana una lista **top-level `inconsistencies[]`** que **agrega todas** las inconsistencias del run (las de archivo —`missing_file`, `unexpected_file`— y, replicadas o referenciadas, las de columna —`missing_column`, `unexpected_column`—), cada una con `type` (del vocabulario cerrado de 4) y `detail` legible no vacío. Así `unexpected_file` tiene por fin un hogar estructural con `{type, detail}`. La lista `unexpected_files` (nombres, orden alfabético) **se conserva** sin cambios (línea 142). Las inconsistencias de columna siguen viviendo además anidadas en `datasets[].files[].inconsistencies` (no se rompe el esquema previo; es aditivo). CA-16 se verifica sobre la lista top-level `inconsistencies[]`.
- **Consecuencias:** Toca un esquema ya aprobado en GATE (por eso es ADR explícito, no cambio silencioso). Es **aditivo** (no elimina campos), por lo que los casos 1-15 ya cerrados no cambian de comportamiento observable salvo la aparición del nuevo campo agregado (que en el caso feliz es `[]`). El caso 16 nace **rojo** y el `tdd_coder` implementa la agregación. Los casos 17 (summary) y siguientes deben tener en cuenta el nuevo campo. Salda la deuda registrada en los `refactor_note` de los casos 11/12.

### D-079 — Política de gobernanza: rama por feature (`feature/<nombre>`) + merge por PR tras prueba humana
- **Estado:** Aceptada — **Implementada por T-034** (2026-07-09)
- **Fecha:** 2026-07-08
- **Contexto:** Al preparar una prueba humana de `ingestion` se detectó que la feature, pese a estar CONFORME a nivel unitario+integración (D-078, `verification.md`), NO es ejecutable de punta a punta vía la CLI publicada: `src/foda/orchestrator.py` no la registra en `FLOWS` (ver L-051). Esto motivó revisar cómo llega una feature a `main`: hoy todo el trabajo de la cadena SDD/TDD ocurre directamente sobre `main`, sin un paso de prueba humana obligatorio antes de integrar. Se acordó con el humano introducir una gobernanza de ramas + PR + prueba humana como puerta de entrada a `main`.
- **Decisión:** (a) Toda feature nueva se trabaja en su propia rama, con nombre `feature/<nombre>` (p. ej. `feature/cli_wiring`). (b) El merge a `main` se hace vía Pull Request usando `gh` (el PR es el artefacto de revisión), con `gh pr merge --merge --delete-branch` tras que la prueba humana sea aprobada. (c) `state.json` gana dos stages terminales nuevos: `human_test` y `merge_to_main` (status pending/done, gate: human), después de `spec_verifier`. Flujo terminal acordado: `spec_verifier` CONFORME → abrir PR → prueba humana → (si pasa) merge a `main` + borrar rama. Queda un punto de alcance ABIERTO sin resolver: si el propio cambio de gobernanza que implementa esta política (T-034) se aplica directo a `main` o se construye en su propia rama.
- **Consecuencias:** Ninguna de las 7 piezas de implementación se ha aplicado todavía (T-034, backlog de alta/media prioridad): `CLAUDE.md` (nueva sección de ramas), `feature_definer.md` (paso 0 crear rama + stages nuevos), `session_closer.md` (push a rama actual `git push -u origin HEAD` en vez de `origin main`), `session_starter.md` (avisar si está en `main` con feature en progreso), `methodology.md` (operacionalizar), `decisions.md` (este ADR) y la plantilla `state.json`. Hasta que T-034 se implemente, el cierre de sesión sigue empujando a `main` (ver A-018). `ingestion` (o `T-032`, cableado del CLI) es candidata a ser la primera feature en estrenar este flujo una vez implementado.
- **Nota de implementación (2026-07-09, T-034):** las 9 piezas (7 originales + `sdd_tdd_workflow.md` + `spec_verifier.md`, ampliación resuelta con el humano, ver L-058) ya están aplicadas: `CLAUDE.md` §3, `feature_definer.md` paso 0 + stages terminales, `session_closer.md` push a `HEAD`, `session_starter.md` paso 3 de verificación de rama, `methodology.md` apéndice operacionalizado, `600_features/_template/state.json` con `human_test`/`merge_to_main`, `spec_verifier.md` (CONFORME avanza a `human_test`, no cierra `completed`), `700_architecture/sdd_tdd_workflow.md` v0.4 (tercer gate humano terminal). El punto de alcance abierto queda resuelto por D-081: T-034 se aplicó directo sobre `main`, sin rama propia.

### D-080 — Gate de progresión entre flujos por `success` del predecesor + `--force`; `foda run` refleja `success` en el exit code
- **Estado:** Aceptada — punto 4 (exit code) **implementado por T-035** (2026-07-09, código+test; prueba humana pendiente); puntos 1-3 (gate + `--force`) **IMPLEMENTACIÓN PENDIENTE** (ver T-036)
- **Fecha:** 2026-07-09
- **Contexto:** Durante la prueba humana de `ingestion` vía CLI (T-033), el usuario ejecutó 6 caminos no-felices y confirmó que todos producían el `ingestion_report.json` esperado con `success:false`, pero notó que `foda run` salía siempre con código 0 salvo en el único caso de fallo duro (`FlowContractError` por artefacto ausente). Esto significa que hoy nada impide encadenar flujos aunque el predecesor haya terminado con inconsistencias: un operador o script que solo mire el exit code no se enteraría. El humano definió una política de gobernanza para cuando exista un flujo consumidor de un reporte con campo `success` (desde `ingestion` en adelante).
- **Decisión:** (1) **Gate por `success` del predecesor**: ningún flujo puede ejecutarse si su predecesor no terminó con `success == true`, leyendo ese campo del reporte del predecesor (p. ej. `ingestion_report.json`). (2) Si no hay OK: `foda run` del flujo siguiente **falla** con código de salida 1 y mensaje claro, **sin escribir nada**. (3) **Override `--force`**: permite correr de todos modos, dejando una advertencia registrada (elección explícita del humano: "fallar + permitir `--force`", no "advertir y continuar" ni "fallar sin excepción"). (4) `foda run` debe **reflejar `success` en el exit code**: sale 0 **solo** si el flujo terminó con `success:true`, 1 si `success:false` (corrige el hallazgo de esta sesión; elección explícita del humano: "sí, corregirlo"). (5) **Excepción temporal — `discovery`/`onboarding`**: `discovery` no está construido (se opera a mano, sin reporte con `success` que leer); `onboarding` solo exige la PRESENCIA de `contract_data.json` (inyección manual), no un "OK de discovery" formal, hasta que `discovery` exista.
- **Consecuencias:** El punto (4) es aislable del resto (no depende de que exista un flujo consumidor) y se registra como tarea separada de alta prioridad (T-035). Los puntos (1)-(3) requieren un flujo consumidor real de un reporte con `success` para tener sentido de implementar (por NC-2/NC-4, no se construye un gate genérico sin un caso de uso concreto que lo ejercite); se registran como T-036, a abordar junto con el primer flujo posterior a `ingestion`. `onboarding → ingestion` ya cumple la regla hoy sin cambios (onboarding es "todo o nada": si falla no escribe el mapa, y el `requires` de `ingestion` ya corta la ejecución si falta `map_client_data.json`); el gate por `success` solo se materializa para sucesores de un flujo que puede escribir salida con `success:false` (`ingestion` en adelante, no `onboarding`).

### D-081 — Enmienda a D-079: alcance provisional (todo a `main`) y contrato "hasta el PR, humano aprueba el merge"
- **Estado:** Aceptada
- **Fecha:** 2026-07-09
- **Contexto:** D-079 dejó un punto de alcance ABIERTO sin resolver: si el propio cambio de gobernanza (T-034) se aplica directo a `main` o en su propia rama. Además, tras registrar T-035 como próxima tarea de código, hacía falta aclarar el contrato general de la futura política de ramas antes de seguir acumulando trabajo directo sobre `main`.
...
### D-088 — Próxima feature = `profiling` banda `stab_1`
- **Estado:** Aceptada
- **Fecha:** 2026-07-09
- **Contexto:** Tras cerrar `profiling` tracer_bullet en `main` (D-087), el pipeline queda con `onboarding`/`ingestion`/`profiling` construidos como tracer_bullets HUECOS (profiling hoy solo escribe `{success:true}` sin calcular nada). Se evaluaron 4 bifurcaciones: profundidad (dar valor real a `profiling`), amplitud (siguiente flujo, `cleaning`), núcleo LLM (`discovery`, bloqueado por T-020 sin rúbricas diseñadas), o consolidación menor.
- **Decisión:** El usuario eligió PROFUNDIDAD: construir `profiling` banda `stab_1` con salud de datos real. Razones: (1) entrega el primer valor analítico tangible del proyecto; (2) es determinista, bajo riesgo (a diferencia de `discovery`); (3) da terreno concreto para el primer bucle de evaluación offline sobre una salida real (conecta con el interés del humano en observabilidad/evaluación, discutido en la misma sesión).
- **Consecuencias:** `cleaning` (050), `discovery` (010, LLM) y la consolidación menor quedan diferidos. Nueva tarea T-039 (alta prioridad). El alcance concreto de `stab_1` se resuelve en D-089.

### D-089 — `profiling` `stab_1`: alcance base — Opción A, salud ESTRUCTURAL
- **Estado:** Aceptada
- **Fecha:** 2026-07-09
