...
| ID | Tarea | Prioridad | Notas |
|---|---|---|---|
| T-014 | Construir feature `client_context` (resolución de rutas, cliente nuevo vs. recurrente) ejecutando la cadena de 8 agentes | 🔴 Alta | Depende de T-013 (`client_scaffold`, completada) y de T-023 (`client_new_cli`, completada). Siguiente en el orden abajo-hacia-arriba acordado en D-016. Próximo paso: invocar `feature_definer`. |

## 5. Backlog
| ID | Tarea | Notas |
|---|---|---|
| T-015 | Construir feature `flow_base` (abstracción `Flow`: load_inputs → validate → execute → write_outputs) | Depende de T-014 (`client_context`, pendiente). Orden abajo-hacia-arriba acordado en D-016. |
