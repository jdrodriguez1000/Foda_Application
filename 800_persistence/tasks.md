...
|---|---|---|---|
| T-023 | Construir la feature `client_new_cli` (`foda client new <NAME>`, cierra el hallazgo F-2 de `client_scaffold`) ejecutando la cadena de 8 agentes | 🔴 Alta | Cadena SDD/TDD hasta `plan_builder` completa con ambos GATEs humanos APROBADOS: 1) `feature_definer` ✅ (5 HU); 2) `spec_writer` ✅ (10 CA, GATE APROBADO); 3) `plan_builder` ✅ (15 TSK, 12 casos TDD, GATE APROBADO). Decisiones vinculantes: D-034 (cadena SDD/TDD completa, no cableado directo), D-035 (la CLI sí lleva tests propios pese al No-Objetivo previo de `client_scaffold`, NC-5 prevalece), D-036 (`clients_root` se resuelve buscando `pyproject.toml` hacia arriba desde el cwd). **SUSPENDIDA** antes de arrancar el bucle TDD por decisión del humano; `state.json` con bloque `suspension` (`resume_at`: invocar `tdd_tester` con el caso 1). **Próximo paso:** invocar `tdd_tester` con el caso 1. |
