...
| ID | Tarea | Prioridad | Notas |
|---|---|---|---|
| T-015 | Construir feature `flow_base` (abstracción `Flow`: load_inputs → validate → execute → write_outputs, consumiendo `ClientContext` vía `Flow.run(ctx)`) ejecutando la cadena de 8 agentes | 🔴 Alta | Depende de T-014 (`client_context`, completada). Siguiente en el orden abajo-hacia-arriba acordado en D-016. Próximo paso: invocar `feature_definer`. |
