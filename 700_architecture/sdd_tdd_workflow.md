...
**Versión:** 0.4 · **Fecha:** 2026-07-09 · Ver [D-008](../800_persistence/decisions.md).

> **Cambios v0.4:** se añaden las dos **etapas terminales de gate humano** `human_test` y `merge_to_main` tras `spec_verifier` (flujo de rama por feature + merge por PR): `spec_verifier` CONFORME ya **no** cierra la celda en `completed`, sino que avanza a `human_test`; la sesión principal abre el PR y el humano prueba y mergea. Ver [D-079](../800_persistence/decisions.md)/[D-081](../800_persistence/decisions.md) y `CLAUDE.md §3`.
                     ...
                     (GATE humano)       (GATE humano)         │             │
                          ...
                          ¿quedan casos pending?  ── sí ──▶ vuelve a tdd_tester
                                          ...
                                          CONFORME                        NO CONFORME
                                               │                    blocked + etapa de retorno
                                               ▼
                                   sesión principal abre PR (gh pr create)
                                               │
                                               ▼
                                      human_test  🚧 (humano prueba vía CLI)
                                               │
                                               ▼
                                      merge_to_main  🚧 (humano mergea el PR)
                                               │
                                               ▼
                                       feature = done (cerrada en main)
```
...
**Flujo de rama por feature (`D-079`/`D-081`; detalle en `CLAUDE.md §3`).** Toda la cadena corre sobre la rama `feature/<nombre>` que crea `feature_definer` (paso 0). Tras `spec_verifier` CONFORME, la **automatización llega hasta el PR** —la sesión principal lo abre con `gh pr create`— y las dos etapas terminales `human_test` y `merge_to_main` son **gates humanos**: el harness **nunca mergea** a `main` por su cuenta.

---

## 4. Gates Humanos

Hay **tres gates obligatorios**. Los dos primeros van **antes de escribir código**; el tercero es **terminal**, antes de integrar a `main`:
...
3. **Terminal, tras `spec_verifier` CONFORME** — rama por feature + merge por PR (`D-079`/`D-081`, `CLAUDE.md §3`): la sesión principal abre el PR y el humano **prueba la feature** (`human_test`) y **mergea el PR a `main`** (`merge_to_main`). El harness nunca mergea por su cuenta.

Protocolo del gate (aplica a los gates 1 y 2; el gate terminal se detalla en `CLAUDE.md §3`):
...
Las dos etapas terminales `human_test` y `merge_to_main` son **gates humanos posteriores a `spec_verifier`** (rama por feature + merge por PR, `D-079`/`D-081`): `spec_verifier` CONFORME avanza `current_stage` a `human_test`; la feature no queda `done` hasta que el humano prueba (`human_test`) y mergea el PR a `main` (`merge_to_main`).
...
| `spec_verifier` | estado + spec/def | CONFORME: `spec_verifier=done`, `current_stage=human_test`, feature `status=in_progress` (aún no cerrada). NO CONFORME: `blocked` + etapa de retorno |
| `human_test` (humano) | — | tras probar la feature vía CLI: `human_test=done`, `current_stage=merge_to_main` |
| `merge_to_main` (humano) | — | tras mergear el PR a `main`: `merge_to_main=done`, feature `status=done`, `current_stage=completed` |

**Reglas de transición clave:**
- Ningún agente avanza `current_stage` a través de un gate: eso lo hace la sesión principal tras la aprobación humana. Los gates son tres: tras `spec_writer`, tras `plan_builder`, y el terminal `human_test`/`merge_to_main` (rama por feature + PR, `D-079`/`D-081`).
- Entre `spec_verifier` CONFORME y `human_test`, la **sesión principal abre el PR** (`gh pr create`); el **merge a `main` lo hace el humano** (`merge_to_main`), nunca el harness.
