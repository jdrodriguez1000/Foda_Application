...
def test_main_sin_name_termina_con_codigo_2(tmp_path, monkeypatch, capsys):
    """Caso 10 (Casos Limite spec, DS-CLI-3): ante argumento NAME ausente
    (main(["client","new"])), argparse imprime usage a stderr y termina con
    codigo 2 (convencion de argparse)."""
    (tmp_path / "pyproject.toml").write_text("", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
